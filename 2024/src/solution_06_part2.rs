...
pub fn run(input: &str){
    ...
    while !move_normal(&mut curr_x, &mut curr_y, curr_dir, width, height, &mut map, &mut history_position) {
    ...
    for (x, y) in &history_position {
        ...
        loop {
            let (a, b) = move_loop(&mut curr_x, &mut curr_y, curr_dir, width, height, &mut map, &mut history_full);
...
fn move_normal(
    ...
    history_position: &mut HashSet<(usize, usize)>) -> bool {
    if curr_dir == 0 {
