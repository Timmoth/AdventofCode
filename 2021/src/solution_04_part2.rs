...
pub fn run() {
    ...
    let mut bingo_number_index = 0;

    let mut winning_boards: Vec<bool> = vec![false; (board_count) as usize];
    ...
    for bingo_number in &bingo_numbers{
        ...
        if let Some(line_indices) = bingo_number_boards.get(*bingo_number as usize) {
            for &line_index in line_indices {
                ...
                if  winning_lines[line_index] <= 0 && !winning_boards[line_index / 10] {
                    winning_line_index = line_index as i32;
                    winning_boards[line_index / 10] = true;
                    last_called_bingo_number_index = bingo_number_index;
                }
            }
        }
    }
